...
public class  TestEmployeeFileIO  {
	...
	private Vector empty, employees;
	...
	public boolean testRead() throws IOException {
		...
		try {
			...
			Vector resultEmpty = EmployeeFileIO.read(fileNameEmpty);

			test = test && assertTrue("1, testing method read with an empty file",
			                          resultEmpty instanceof Vector);
			test = test && assertTrue("2, testing method read with an empty file",
			                          resultEmpty.size() == 0);

			// Testing a not empty file
			Vector result = EmployeeFileIO.read(fileNameEmployees);
