...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		...
		/* HERE IS MY CODE */
		int numOrders = 0;
		for(Iterator it1 = sales.getOrdersIterator(); it1.hasNext(); )

			for(Iterator it2 = ((Order)it1.next()).getItemsIterator(); it2.hasNext(); ){

				if(product.equals(((OrderItem)it2.next()).getProduct())){
	...
	public void displayTotalQuantityOfProducts() {
        ...
        int count = catalog.getNumberOfProducts();
        ...
        for(Iterator itProduct = catalog.getProductsIterator(); itProduct.hasNext();){

            Product product = (Product)itProduct.next();
            for(Iterator itOrder = sales.getOrdersIterator(); itOrder.hasNext();){

                Order order = (Order)itOrder.next();
                for(Iterator itItem = order.getItemsIterator(); itItem.hasNext();){

                    OrderItem item = (OrderItem)itItem.next();
                    Product newProduct = item.getProduct();
                    if(product.equals(newProduct)){

                        numOfProducts[i] += item.getQuantity();
                    }
                }
            }
        ...
        for(Iterator itCatalog = catalog.getProductsIterator(); itCatalog.hasNext();){

            Product product = (Product)itCatalog.next();
            stdOut.println(product.getCode() + " " + numOfProducts[i]);
            i++;
